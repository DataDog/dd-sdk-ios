...
public class URLSessionInterceptor: URLSessionInterceptorType {
    ...
    internal let traceIDGenerator: TraceIDGenerator
    ...
    init(
        ...
        self.traceIDGenerator = configuration.traceIDGenerator
    ...
    private var interceptionByTask: [URLSessionTask: URLSessionTaskInterception] = [:]
    ...
    public func taskCreated(task: URLSessionTask, session: URLSession? = nil) {
        ...
        queue.async {
            ...
            if let ids = self.extractSpanContext(from: request, session: session) {
    ...
    private func finishInterception(task: URLSessionTask, interception: URLSessionTaskInterception) {
    ...
    private func extractSpanContext(from request: URLRequest, session: URLSession?) -> (
        traceID: TraceID,
        spanID: SpanID,
        parentSpanID: SpanID?
    )? {
