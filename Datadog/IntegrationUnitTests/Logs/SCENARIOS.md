...
- **`date` matches simulated time** — log emitted at simulated time T carries `date` field equal to T; verifies that harness time-mocking flows through to the log payload. _ready_ (in-test bridge `config.dateProvider = app.dateProvider`; see Observations & Notes "in-test feature-config bridges") → `testGivenSimulatedTime_whenLogIsEmitted_itsDateMatchesSimulatedTime()`
...
- **Debug launch argument forces 100% sample rate** — when `LaunchArguments.Debug` is in process arguments, `remoteSampleRate` (any non-zero value) is overridden to 100%. Note: the early guard `remoteSampleRate > 0` runs before the debug-override, so `remoteSampleRate=0` still produces a NOPLogger. _ready_ → `testGivenDebugLaunchArgument_whenLoggerWithNonZeroSampleRateEmits_allLogsAreRecorded()`
...
- **`networkInfoEnabled=true` + WiFi reachability** — log carries `network.client.reachability="yes"`, `available_interfaces` includes "wifi", and connection-meta fields: `supports_ipv4`, `supports_ipv6`, `is_expensive`, `is_constrained`, `link_quality`. _needs-fixture: network state mock_ → `testGivenNetworkInfoEnabledAndWiFiReachability_whenLogIsEmitted_logCarriesAllNetworkClientFields()`
- **`networkInfoEnabled=true` + cellular + carrier** — log carries cellular reachability and `network.client.sim_carrier.*` (name, iso_country, technology, allows_voip). _needs-fixture: network state mock_ → `testGivenNetworkInfoEnabledAndCellularWithCarrier_whenLogIsEmitted_logCarriesSimCarrierFields()`
- **Reachability change between logs** — log A emitted while online; log B emitted while offline; reflects different `reachability`. _needs-fixture: network state mock_ → `testGivenNetworkInfoEnabled_whenReachabilityChangesBetweenLogs_eachLogReflectsItsReachability()`
...
- **`DD_DEBUG` launch argument cannot resurrect a `remoteSampleRate=0` logger; also: `Logger.Configuration.processInfo` is not wired through `Datadog.Configuration`** (surfaced in Batch 17, §7 "Debug launch argument forces 100"). Two separate findings: (1) `Logger.create` short-circuits the `RemoteLogger` construction with an early `guard configuration.remoteSampleRate > 0 else { return nil }` (`DatadogLogs/Sources/Logger.swift:152`), and only *inside* the constructed `RemoteLogger` does the `Sampler(samplingRate: debug ? 100 : configuration.remoteSampleRate)` rewrite take effect (`Logger.swift:165`). So when `remoteSampleRate=0`, the logger collapses to a NOPLogger before the debug-override gets a chance to run; `DD_DEBUG` only forces 100% acceptance for loggers configured with a non-zero `remoteSampleRate`. SCENARIOS.md description for §7 #3 originally implied `remoteSampleRate=0` is overridden — corrected to reflect the actual pipeline order. (2) `Logger.Configuration.processInfo` is an `internal` field that defaults to `ProcessInfo.processInfo` (the real system one) and is *not* populated from `Datadog.Configuration.processInfo`. So even though `AppRunner.processInfo` is wired into the SDK core via `Datadog.Configuration`, the debug-override path in `Logger.create` reads the *real* process arguments unless the test explicitly assigns `config.processInfo = app.processInfo` on the per-logger config. The harness exposes `app.setProcessArguments(_:)` (and a step factory) for the AppRunner-level mock, but the test author still has to bridge it into `Logger.Configuration` via `@testable import DatadogLogs`. Follow-up: consider either lifting the debug-override above the early guard, or auto-populating `Logger.Configuration.processInfo` from the SDK core context so `DD_DEBUG` actually flows through SDK config.
...
- **Recurring pattern: in-test feature-config bridges from `AppRunner` to `Logger`/`Logs` configuration** (surfaced across Batches 17 and 18, §7 + §3). Two `internal` fields on Logs feature configuration default to system providers and are *not* populated from the SDK core context: `Logger.Configuration.processInfo` (default `ProcessInfo.processInfo`) and `Logs.Configuration.dateProvider` (default `SystemDateProvider()`). `AppRunner` exposes mock equivalents (`app.processInfo`, `app.dateProvider`) wired into `Datadog.Configuration`, but the harness has to bridge them per-test via `@testable import DatadogLogs` — `config.processInfo = app.processInfo` (Batch 17) and `config.dateProvider = app.dateProvider` (Batch 18). Pattern is identical and adds 1 line per test that needs simulated time / simulated process arguments to flow into the Logs feature. Follow-up: consider auto-bridging both fields from the SDK core context at `Logs.enable(...)` / `Logger.create(...)` time so harness mocks (and any user-supplied custom providers on `Datadog.Configuration`) flow through without per-call duplication.
