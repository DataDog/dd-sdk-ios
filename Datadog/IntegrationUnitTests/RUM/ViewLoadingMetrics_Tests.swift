...
// TODO: RUM-16908 Merge into `ViewLoadingMetrics` once `viewUpdates` feature flag is removed
...
/// Mirrors `ViewLoadingMetricsTests` with `featureFlags[.viewUpdates] = true`.
class ViewLoadingMetrics_Tests: XCTestCase {
    ...
    func testWhenActionOccursInPreviousView_andNextViewStarts_thenINVIsTrackedForNextView() throws {
        ...
        let actualINV = try XCTUnwrap(nextView.viewEvents.first?.view.interactionToNextViewTime)
        XCTAssertEqual(TimeInterval.ddFromNanoseconds(actualINV), expectedINV, accuracy: 0.01)
    }
    ...
    func testWhenActionOccursInPreviousView_andNextViewStartsAfterThreshold_thenINVIsNotTrackedForNextView() throws {
        ...
        let nextView = try XCTUnwrap(session.views.first(where: { $0.name == "NextView" }))
        let actualINV = nextView.viewEvents.first?.view.interactionToNextViewTime
    ...
    func testWhenActionInPreviousViewIsDropped_thenINVIsNotTracked() throws {
        ...
        let nextView = try XCTUnwrap(session.views.first(where: { $0.name == "NextView" }))
        XCTAssertNil(nextView.viewEvents.first?.view.interactionToNextViewTime, "The INV value should not be tracked when the action is dropped.")
    }
    ...
    func testINVIsOnlyTrackedForViewsThatWereStartedDueToAnAction() throws {
        ...
        let nextView = try XCTUnwrap(session.views.first(where: { $0.name == "NextView" }))
        let nextViewWithoutAction = try XCTUnwrap(session.views.first(where: { $0.name == "NextViewWithoutAction" }))
        XCTAssertNotNil(nextView.viewEvents.first?.view.interactionToNextViewTime, "INV should be tracked for view that started due to an action.")
        XCTAssertNil(nextViewWithoutAction.viewEvents.first?.view.interactionToNextViewTime, "INV should not be tracked for view that started without an action.")
    }
    ...
    func testGivenCustomINVActionPredicate_whenNextViewStarts_thenINVMetricIsCalculatedFromClassifiedActions() throws {
        ...
        // Find the first `view` event for "WelcomeView" (INV is set at view open)
        let nextView = try XCTUnwrap(session.views.first(where: { $0.name == "WelcomeView" }))

        // Check the INV metric
        let actualINV = try XCTUnwrap(nextView.viewEvents.first?.view.interactionToNextViewTime)
        XCTAssertEqual(
            TimeInterval.ddFromNanoseconds(actualINV),
            expectedINV,
            accuracy: 0.01,
