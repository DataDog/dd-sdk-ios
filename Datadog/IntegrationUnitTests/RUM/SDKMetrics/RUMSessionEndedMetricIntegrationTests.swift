...
class RUMSessionEndedMetricIntegrationTests: XCTestCase {
    ...
    func testTrackingNTPOffset() throws {
        ...
        // Then
        let metric = try XCTUnwrap(core.waitAndReturnSessionEndedMetricEvent())
        XCTAssertEqual(metric.attributes?.ntpOffset.atStart, offsetAtStart.dd_toInt64Milliseconds)
        XCTAssertEqual(metric.attributes?.ntpOffset.atEnd, offsetAtEnd.dd_toInt64Milliseconds)
    }
...
private extension Int64 {
    var nanosecondsToSeconds: TimeInterval { TimeInterval(dd_fromNanoseconds: self) }
}
