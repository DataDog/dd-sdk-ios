...
class DatadogEventBridgeTests: XCTestCase {
    ...
    func testWhenLogMessageIsValid_itSucceedsParsing() throws {
        let json = try DatadogEventBridge.parse(messageLog)
        XCTAssertEqual(json[DatadogEventBridge.Constants.eventTypeKey] as? String, "log")
    }

    func testWhenRUMMessageIsValid_itSucceedsParsing() throws {
        let json = try DatadogEventBridge.parse(messageRUM)
        XCTAssertEqual(json[DatadogEventBridge.Constants.eventTypeKey] as? String, "view")
    }
    ...
    func testWhenMessageIsInvalid_itFailsParsing() {
        ...
        XCTAssertThrowsError(
            try DatadogEventBridge.parse(messageInvalidJSON),
