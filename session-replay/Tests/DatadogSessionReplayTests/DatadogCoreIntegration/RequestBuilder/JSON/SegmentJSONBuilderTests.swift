...
class SegmentJSONBuilderTests: XCTestCase {
    ...
    private func generateEnrichedRecordJSONs(for segment: SRSegment) throws -> [EnrichedRecordJSON] {
        ...
        let chunkedRecords = segment.records.chunkedRandomly(numberOfChunks: .random(in: 1...segment.records.count))
        let enrichedRecords = chunkedRecords.map { EnrichedRecord(rumContext: rum, records: $0) }
        return try enrichedRecords.map { try EnrichedRecordJSON(jsonObjectData: try encode($0)) }
    }
