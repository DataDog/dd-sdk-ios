...
internal protocol NetworkContextProvider: AnyObject {
    ...
    /// Notifies on `NetworkContext` change.
    var onNetworkContextChange: (NetworkContext) -> Void { set get }
}
...
internal class NetworkContextCoreProvider: NetworkContextProvider {
    /// Queue for synchronizing `unsafeNetworkContext` updates.
    private let queue = DispatchQueue(
        label: "com.datadoghq.network-context",
        target: .global(qos: .utility)
    )

    /// Unsafe callback instance.
    private var _callback: (NetworkContext) -> Void = { _ in }

    /// Unsychronized `NetworkContext`. The `queue` must be used to synchronize its mutation.
    private var _context: NetworkContext? {
    ...
    var onNetworkContextChange: (NetworkContext) -> Void {
...
extension NetworkContextCoreProvider: FeatureMessageReceiver {
    ...
    private func update(context: DatadogContext, to core: DatadogCoreProtocol) {
        queue.async { [weak self] in
            ...
            do {
                self._context = NetworkContext(rumContext: try context.baggages["rum"]?.decode())
            } catch {
                core.telemetry
...
extension NetworkContextCoreProvider: Flushable {
