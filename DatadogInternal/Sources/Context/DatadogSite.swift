...
extension DatadogSite {
    ...
    /// The CDN hostname used for fetching remote configuration documents.
    /// Format: `sdk-configuration.<intake-host>`
    ///
    /// The full URL (with path and ID) is constructed by `RemoteConfigurationSynchronizer`.
    @_spi(Internal)
    public var remoteConfigurationHost: String? {
        endpoint.host.map { "sdk-configuration.\($0)" }
    }
}
