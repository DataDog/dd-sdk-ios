...
internal struct TracingURLSessionHandler: DatadogURLSessionHandler {
    ...
    let tracingSampler: DatadogInternal.Sampler
    ...
    init(
        ...
        tracingSampler: DatadogInternal.Sampler,
    ...
    func interceptionDidComplete(interception: DatadogInternal.URLSessionTaskInterception) {
        ...
        if let requestUrl = interception.request.url {
            ...
            span.setAttribute(key: DatadogSpanTag.resource, value: resourceUrl)
        }
        let method = interception.request.httpMethod ?? "unknown_method"
        span.setAttribute(key: OTTags.httpUrl, value: url)
        span.setAttribute(key: OTTags.httpMethod, value: method)
        ...
        if let httpResponse = resourceCompletion.httpResponse {
            ...
            span.setAttribute(key: OTTags.httpStatusCode, value: httpStatusCode)
            if let error = httpResponse.asClientError() {
                span.status = .error(description: "")
                if httpStatusCode == 404 {
                    span.setAttribute(key: DatadogSpanTag.resource, value: "404")
                }
            }
        }
        ...
        if let history = contextReceiver.context.applicationStateHistory {
            ...
            span.setAttribute(key: DatadogSpanTag.foregroundDuration, value: Int(appStateHistory.foregroundDuration.toNanos))
            ...
            span.setAttribute(key: DatadogSpanTag.isBackground, value: didStartInBackground || doesEndInBackground)
        }
