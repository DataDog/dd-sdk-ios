...
sdk_version: 3.13.0
verified_against_commit: c418bbf4c
tracked_files:
...
- **`DatadogTrace/Sources/OpenTracing/OTSpan.swift`** — OpenTracing span protocol (`Sendable`), two required tag-setting methods: `setTag(key:value: OTTagValue)` and `setTag(key:value: [String: OTTagValue])`; also `log(fields:)` (fields are `[String: Encodable & Sendable]`), `setBaggageItem`, `setActive`, `finish`, `setError`, `keepTrace`, `dropTrace`.
  ...
  - Flattening a dictionary tag does not clear any pre-existing tag already stored at the container key: e.g. `setTag(key: "context", value: "old")` followed by `setTag(key: "context", value: ["foo": "bar"])` leaves both `context = "old"` and `context.foo = "bar"` set. Pre-existing characteristic of the tag store (a flat `[String: OTTagValue]`), not something flattening introduced or fixed.
