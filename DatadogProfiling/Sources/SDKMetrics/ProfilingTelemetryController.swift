...
internal final class ProfilingTelemetryController {
    ...
    init(telemetry: Telemetry = NOPTelemetry(), configMetric: ConfigurationMetric = ConfigurationMetric()) {
    ...
    func send(metric: MetricAttributesConvertible) {
        ...
        telemetry.metric(name: metric.metricName, attributes: metricAttributes)
    }
}
