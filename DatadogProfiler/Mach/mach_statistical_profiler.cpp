 ...
 */
uint32_t mach_statistical_profiler::get_sampling_interval() const {
    ...
    uint32_t base_interval = config.sampling_interval_ms;
    uint32_t jitter_range = (base_interval * stat_config.jitter_percentage) / 100;
    ...
    // Ensure we don't go below 1ms minimum interval
    int32_t jittered_interval = static_cast<int32_t>(base_interval) + jitter;
    return static_cast<uint32_t>(std::max(1, jittered_interval));
}
