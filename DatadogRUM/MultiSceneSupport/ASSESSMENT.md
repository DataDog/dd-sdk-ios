...
pass 65/65 tests (`EXP-108` through `EXP-122`). The exact-scene manual-authority
regression set passes 8/8. The first complete RUM run passed 1,164/1,165; its
...
| Normal-app compatibility | Targeted manual-authority tests pass 8/8 and the clean complete RUM rerun passes 1,165/1,165; Trace remains 151/151. Complete Internal/Logs/WebView suites at their stated checkpoints, probe builds, 65/65 native probe tests, package build, and repository lint pass | Live single-scene behavior, custom-handler integration, and `sendEvent` overhead/recursion measurement |
...
scene-aware overload review and live same-key A/B proof remain.
Real-device and
