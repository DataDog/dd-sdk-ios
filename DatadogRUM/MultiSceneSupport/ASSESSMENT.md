...
pass 68/68 tests (`EXP-108` through `EXP-125`). The exact-scene manual-authority
...
passes 1,169/1,169 and the probe passes 68/68. This closes the internal Sheet
path, not full-screen-cover parity or the reviewed public semantic API.
...
subtree. It does not yet prove full-screen cover or approve a public API.
...
14/14 plus exact backend intake. Sheet is now internally correct; full-screen
cover and the reviewed public complete-destination integration remain.
Real-device and
...
| Automatic SwiftUI view lifetime | Transparent hosting-controller discovery can select process-global history, run after lifecycle work, miss semantic route changes, or preempt explicit authority | Scene-keyed routing and keyed occurrence state pass. `EXP-115` adds weak target-scoped authority while keeping automatic tracking enabled elsewhere; `EXP-116` centralizes path/resolver ownership; `EXP-122` integrates targeted manual views with the scene stack; `EXP-125` adds a suppression-only state mounted in the semantic presentation subtree | Automatic-only semantic stack/split baselines remain wrong. Explicit coexistence passes H1/D1/H2; manual coexistence passes H1/M1/H2. `EXP-123` proves handler authority alone still emits an automatic Sheet; `EXP-125` passes 14/14 with H1/Sheet-M1/fresh-H2 and no automatic Sheet. Probe tests pass 68/68; focused presentation/manual tests pass 6/6; full RUM passes 1,169/1,169 | Backend distinguishes the wrong automatic baselines from correct semantic chains. `EXP-125` confirms five total views including startup, exact Sheet and dismiss owners, and zero errors/crashes. `EXP-118` remains hardware-inconclusive | Automatic discovery remains the zero-code default. Internal targeted manual and Sheet authority pass. Full-screen cover, public manual/semantic APIs, sibling-container isolation, live same-key A/B, physical scene coexistence, and normal-app validation remain |
| SwiftUI navigation | Hosting controllers from all windows share history, transparent discovery may report containers instead of customer destinations, and cancellation can publish an uncommitted path | A bound typed path drives RUM occurrence generations; retained routes use guarded scene proof; transition completion owns commit/cancel. The `EXP-116` wrapper centralizes route metadata. `EXP-125` extends the complete-destination model to Sheet while separating semantic authority from native subtree lifetime | Route-owned return, abort, replacement, split selection, retained return, and Sheet present/dismiss pass. `EXP-125` starts fresh H2 before immediate dismiss work and rejects the automatic Sheet. `EXP-100` gestures were ignored, so native interactive source delivery remains unproven | Backend confirms distinct repeated-destination UUIDs, exact stack/split owners, and the `EXP-125` H1/Sheet-M1/H2 chain | Occurrence mechanics, once-per-container integration shape, and Sheet presentation pass experimentally. Full-screen cover, reviewed API, human gestures, adaptive navigation, sibling containers, simultaneous-visible hardware, and restoration remain |
