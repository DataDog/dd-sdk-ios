...
pass 133/133 tests (`EXP-108` through `EXP-140`). The exact-scene manual-authority
...
that harness gap. The full RUM suite passes 1,171/1,171, the full probe passes
133/133, Objective-C selector smoke and fallback tests pass, lint passes, and the
SPI builds in Release under Xcode 27. This validates prototype usefulness, not
stable API approval.
...
| Normal-app compatibility | Exact manual-authority tests pass 8/8, the presentation/manual contract set passes 6/6, and the current complete RUM rerun passes 1,171/1,171; Trace remains 151/151. Complete Internal/Logs/WebView suites at their stated checkpoints, probe builds, 133/133 native probe tests, package build, and repository lint pass. The customer-shaped manual SPI also passes an Xcode 27 Release build | Live single-scene behavior, custom-handler integration, and `sendEvent` overhead/recursion measurement |
...
| Automatic SwiftUI view lifetime | Transparent hosting-controller discovery can select process-global history, run after lifecycle work, miss semantic route changes, or preempt explicit authority | Scene-keyed routing and keyed occurrence state pass. `EXP-115` adds weak target-scoped authority while keeping automatic tracking enabled elsewhere; `EXP-116` centralizes path/resolver ownership; `EXP-122` integrates targeted manual views with the scene stack; `EXP-125`/`EXP-126` mount suppression-only state in independent Sheet and full-screen-cover subtrees; `EXP-127` proves containment across two sibling controller branches under one outer host; `EXP-128` exercises a real nested manual suffix plus duplicate active-key misuse; `EXP-129` adds exact same-key A/B and reverse-stop expectations; `EXP-137` through `EXP-140` route those one-scene flows through the customer-shaped SPI | Automatic-only semantic stack/split baselines remain wrong. Explicit coexistence passes H1/D1/H2; customer-shaped manual coexistence passes H1/M1/H2. Sheet and cover each pass 14/14 with one semantic presentation and fresh H2; nested manual passes 29/29 with H1/C1/P1/fresh-C2/fresh-H2 and no duplicate-key restart. Probe tests pass 133/133; focused presentation/manual tests pass 6/6; full RUM passes 1,171/1,171. `EXP-129` reached both simulator scenes but expired before manual starts | Backend distinguishes the wrong automatic baselines from correct semantic chains. `EXP-137` through `EXP-140` confirm the manual, nested, Sheet, and cover customer API paths with exact action/Resource owners and zero errors/crashes. `EXP-118` and `EXP-129` remain hardware-inconclusive | Automatic discovery remains the zero-code default. The manual API proposal is implemented and experimentally validated; stable review, the semantic-container SPI, live same-key A/B proof, physical scene coexistence, and normal-app validation remain |
