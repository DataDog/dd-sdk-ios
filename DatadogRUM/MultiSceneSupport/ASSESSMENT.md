...
plan passed 1,153/1,153. The SwiftUI authority slice adds four regressions and the
current complete RUM plan passes 1,157/1,157; Trace remains 151/151, including 4/4 focused OpenTelemetry
...
pass 65/65 tests (`EXP-108` through `EXP-120`). Three clean iPadOS 27 Home → Detail → Home runs each
...
| Normal-app compatibility | RUM 1,157/1,157 and Trace 151/151 pass; complete Internal/Logs/WebView suites, probe builds, package build, and repository lint pass | Live single-scene behavior, custom-handler integration, and `sendEvent` overhead/recursion measurement |
...
The next production slice must integrate targeted manual entries with the
handler's per-scene stack, stage automatic candidates underneath an active manual
suffix, and reveal a fresh underlying occurrence on exact scene/key removal.
Real-device and
...
| Automatic SwiftUI view lifetime | Transparent hosting-controller discovery can select process-global history, run after lifecycle work, miss semantic route changes, or preempt a direct keyed manual view | Core scene routing accepts discovered controllers; the route-owned prototype adds keyed state, atomic replacement, disconnect fencing, retained-reader handling, and a weak per-window committed-occurrence source without changing customer content identity. `EXP-115` adds a weak active-subtree authority registry that leaves automatic tracking enabled outside the containing controller and preserves UIKit precedence. `EXP-116` moves path observation and metadata resolution to one probe container while retaining destination-builder placement. `EXP-118` adds scene-selective semantic enablement. `EXP-119` adds an explicit auxiliary Sheet; `EXP-120` exercises the direct keyed API's missing authority | Automatic baselines remain wrong. Route-owned placement passes initial roots/destinations; `EXP-090` passes same-type replacement with one customer state token; `EXP-091` rejects an unmaterialized abort; `EXP-098`/`EXP-099` reveal retained Home before immediate work; `EXP-102`/`EXP-103` pass same-type split replacement in one and two windows; `EXP-105` preserves a source-started returned split occurrence across remount; `EXP-111` deterministically reruns split replacement under both modes and the automatic control fails 0/9. `EXP-115` runs automatic and explicit tracking together and passes 7/7 with only H1/D1/H2. `EXP-116` repeats return, abort, and same-type replacement through the one-container wrapper. Both clean `EXP-118` prefixes keep A exact and create B-specific automatic views after B opens, but simulator `backboardd` aborts prevent the terminal marker. `EXP-119` creates H1/S1/H2 without a duplicate Sheet but fails immediate `onDismiss` ownership. `EXP-120` creates H1/M1/H2 but an automatic fallback interrupts M1 and owns its work. The probe passes 65/65; authority/handler regressions add four RUM tests | Backend distinguishes wrong transparent baselines from correct debug-source stack and split chains. `EXP-115` has exactly launch plus H1/D1/H2 and no automatic duplicate; all three `EXP-116` runs match their local oracle. `EXP-119` confirms the modal return gap. `EXP-120` confirms six views and fallback ownership of active/immediate markers, with zero errors/crashes. `EXP-118` has no acceptance query because neither run reached a terminal result | Automatic discovery remains the zero-code default and target-scoped semantic suppression passes. Direct keyed manual coexistence fails; internal per-scene manual authority/reveal, sibling-container isolation, reviewed APIs, terminal physical-device coexistence, and normal-app validation remain |
| SwiftUI navigation | Hosting controllers from all windows share history, transparent discovery may report containers instead of customer screens, and cancellation can publish an uncommitted path | Scene-keyed scope plus a bound typed path drives RUM-only occurrence generations; retained routes use last concrete scene proof only as guarded fallback; arbiter completion owns commit/cancel; a replacement tracking state can adopt a source-published occurrence. The `EXP-116` wrapper owns root/destination materialization and centralizes route metadata once per stack. `EXP-119` shows that stack-only automatic discovery cannot start a returned destination before immediate modal-dismiss work | Explicit repeated push/pop and native/backend cancellation controls pass; debug source passes different-type, same-type, abort, retained Home, same-type split replacement, and retained split return without `.id`. `EXP-111` drives split cases and passes 10/10 and 13/13. `EXP-116` passes return 7/7, abort 5/5, and same-type replacement 6/6. `EXP-119` produces a fresh returned H2, but only after the immediate callback. `EXP-100` gestures were ignored, so native interactive source delivery is still unproven | `EXP-090` emits distinct Detail UUIDs without customer-state reset; `EXP-098` emits Home₁ -> Detail -> Home₂ with exact immediate work; `EXP-102`/`EXP-103` emit exact split chains; `EXP-105` adds one exact returned-Detail occurrence; `EXP-111` repeats them with local/backend agreement; `EXP-116` preserves exact return, abort, and replacement sets; `EXP-119` confirms the modal return gap in backend intake | Occurrence mechanics and a once-per-container integration shape pass experimentally. API review must cover presentation state or an equivalent reveal; then cover human gestures, adaptive navigation, sibling containers, simultaneous-visible hardware, and restoration |
