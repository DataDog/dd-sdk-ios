...
pass 93/93 tests (`EXP-108` through `EXP-130`). The exact-scene manual-authority
...
| Resources, traces, and operations | Start provenance and Resource completion ownership are frozen; manual Resource starts, automatic URLSession completion, and native/OpenTelemetry span starts use scene handoff; Operations use application-wide typed identities and per-step resolution. `EXP-130` backend-confirms Home→Detail success/failure and duplicate latest-start semantics | Finish bounded live causal/reverse-completion rows, exact different-representative proof, live Operation A-to-B proof, and public Operation target review |
...
| Operation starts before navigation and ends after it | `EXP-130` backend-confirms success H1→D1 and failure H2→D2 with different start/end view UUIDs | Backend-pass for one scene; A-to-B cross-scene completion remains pending |
