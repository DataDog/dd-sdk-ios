...
pass 76/76 tests (`EXP-108` through `EXP-127`). The exact-scene manual-authority
...
The probe now passes 76/76. This closes internal sibling-container isolation; the
...
| Automatic SwiftUI view lifetime | Transparent hosting-controller discovery can select process-global history, run after lifecycle work, miss semantic route changes, or preempt explicit authority | Scene-keyed routing and keyed occurrence state pass. `EXP-115` adds weak target-scoped authority while keeping automatic tracking enabled elsewhere; `EXP-116` centralizes path/resolver ownership; `EXP-122` integrates targeted manual views with the scene stack; `EXP-125`/`EXP-126` mount suppression-only state in independent Sheet and full-screen-cover subtrees; `EXP-127` proves containment across two sibling controller branches under one outer host | Automatic-only semantic stack/split baselines remain wrong. Explicit coexistence passes H1/D1/H2; manual coexistence passes H1/M1/H2. `EXP-123` proves handler authority alone still emits an automatic Sheet; `EXP-125` and `EXP-126` each pass 14/14 with H1/presentation-M1/fresh-H2 and no automatic presentation duplicate. The sibling scenario passes 19/19 and emits no unrelated automatic current view while M1 is active. Probe tests pass 76/76; focused presentation/manual tests pass 6/6; full RUM passes 1,169/1,169 | Backend distinguishes the wrong automatic baselines from correct semantic chains. `EXP-125` and `EXP-126` each confirm five total views including startup, exact presentation/dismiss owners, and zero errors/crashes. Final `EXP-127` confirms four views, exact sibling reveal/owners, and zero errors/crashes. `EXP-118` remains hardware-inconclusive | Automatic discovery remains the zero-code default. Internal targeted manual, presentation, and sibling-container authority pass. Public manual/semantic APIs, live nested/duplicate-manual and same-key A/B proof, physical scene coexistence, and normal-app validation remain |
| SwiftUI navigation | Hosting controllers from all windows share history, transparent discovery may report containers instead of customer destinations, and cancellation can publish an uncommitted path | A bound typed path drives RUM occurrence generations; retained routes use guarded scene proof; transition completion owns commit/cancel. The `EXP-116` wrapper centralizes route metadata. `EXP-125` and `EXP-126` extend the complete-destination model to Sheet and full-screen cover while separating semantic authority from native subtree lifetime. `EXP-127` scopes authority by real descendant-controller containment | Route-owned return, abort, replacement, split selection, retained return, Sheet, and full-screen-cover present/dismiss pass. `EXP-125`/`EXP-126` start fresh H2 before immediate dismiss work and reject the matching automatic presentation. `EXP-127` stages only the latest right-sibling destination beneath M1 and reveals it as a fresh occurrence. `EXP-100` gestures were ignored, so native interactive source delivery remains unproven | Backend confirms distinct repeated-destination UUIDs, exact stack/split owners, independent H1/presentation-M1/H2 chains for both required presentation styles, and H1/M1/fresh-Detail for the sibling-container case | Occurrence mechanics, once-per-container integration shape, required presentation styles, and internal sibling isolation pass experimentally. Reviewed API, human gestures, adaptive navigation, simultaneous-visible hardware, and restoration remain |
