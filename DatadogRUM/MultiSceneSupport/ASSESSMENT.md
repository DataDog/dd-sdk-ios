...
pass 120/120 tests (`EXP-108` through `EXP-134`). The exact-scene manual-authority
...
container. These experiments do not approve a public API.
...
| Resources, traces, and operations | Start provenance and Resource completion ownership are frozen; manual Resource starts, automatic URLSession completion, and native/OpenTelemetry span starts use scene handoff; Operations use application-wide typed identities and per-step resolution. `EXP-130` backend-confirms Home→Detail success/failure and duplicate latest-start semantics; `EXP-131` prepares exact Operation A→B and reverse-completion acceptance; `EXP-133` backend-confirms one Trace-only request across representative churn; `EXP-134` backend-confirms two independent A/B requests completed in reverse order without owner drift | Finish SwiftUI Button/Task, shared/coalesced requests, exact-source hardware proof, run `EXP-131` on capable hardware, and complete public Operation target review |
...
| Normal-app compatibility | Exact manual-authority tests pass 8/8, the presentation/manual contract set passes 6/6, and the clean complete RUM rerun passes 1,169/1,169; Trace remains 151/151. Complete Internal/Logs/WebView suites at their stated checkpoints, probe builds, 120/120 native probe tests, package build, and repository lint pass | Live single-scene behavior, custom-handler integration, and `sendEvent` overhead/recursion measurement |
...
| Automatic SwiftUI view lifetime | Transparent hosting-controller discovery can select process-global history, run after lifecycle work, miss semantic route changes, or preempt explicit authority | Scene-keyed routing and keyed occurrence state pass. `EXP-115` adds weak target-scoped authority while keeping automatic tracking enabled elsewhere; `EXP-116` centralizes path/resolver ownership; `EXP-122` integrates targeted manual views with the scene stack; `EXP-125`/`EXP-126` mount suppression-only state in independent Sheet and full-screen-cover subtrees; `EXP-127` proves containment across two sibling controller branches under one outer host; `EXP-128` exercises a real nested manual suffix plus duplicate active-key misuse; `EXP-129` adds exact same-key A/B and reverse-stop expectations | Automatic-only semantic stack/split baselines remain wrong. Explicit coexistence passes H1/D1/H2; manual coexistence passes H1/M1/H2. `EXP-123` proves that handler authority alone still emits an automatic Sheet; `EXP-125` and `EXP-126` each pass 14/14 with H1/presentation-M1/fresh-H2 and no automatic presentation duplicate. The sibling scenario passes 19/19 and emits no unrelated automatic current view while M1 is active. The nested scenario passes 29/29 with H1/C1/P1/fresh-C2/fresh-H2 and no duplicate-key restart. Probe tests pass 120/120; focused presentation/manual tests pass 6/6; full RUM passes 1,169/1,169. `EXP-129` reached both simulator scenes but expired before manual starts | Backend distinguishes the wrong automatic baselines from correct semantic chains. `EXP-125` and `EXP-126` each confirm five total views including startup, exact presentation/dismiss owners, and zero errors/crashes. Final `EXP-127` confirms four views and exact sibling reveal/owners. `EXP-128` confirms distinct C1/C2 and H1/H2 IDs, no duplicate-start view, exact action/Resource ownership, and zero errors/crashes. `EXP-118` and `EXP-129` remain hardware-inconclusive | Automatic discovery remains the zero-code default. Internal targeted manual, nested-manual, presentation, and sibling-container authority pass. Public manual/semantic APIs, live same-key A/B proof, physical scene coexistence, and normal-app validation remain |
| SwiftUI navigation | Hosting controllers from all windows share history, transparent discovery may report containers instead of customer destinations, and cancellation can publish an uncommitted path | A bound typed path drives RUM occurrence generations; retained routes use guarded scene proof; transition completion owns commit/cancel. The `EXP-116` wrapper centralizes route metadata. `EXP-125` and `EXP-126` extend the complete-destination model to Sheet and full-screen cover while separating semantic authority from native subtree lifetime. `EXP-127` scopes authority by real descendant-controller containment; `EXP-128` applies the same occurrence rule to nested manual destinations; `EXP-129` encodes independent same-key manual occurrences by scene | Route-owned return, abort, replacement, split selection, retained return, Sheet, and full-screen-cover present/dismiss pass. `EXP-125`/`EXP-126` start fresh H2 before immediate dismiss work and reject the matching automatic presentation. `EXP-127` stages only the latest right-sibling destination beneath M1 and reveals it as a fresh occurrence. `EXP-128` creates fresh Compose C2 after Preview and fresh Home H2 after Compose. `EXP-129` remains hardware-inconclusive before manual steps. `EXP-100` gestures were ignored, so native interactive source delivery remains unproven | Backend confirms distinct repeated-destination UUIDs, exact stack/split owners, independent H1/presentation-M1/H2 chains for both required presentation styles, H1/M1/fresh-Detail for the sibling case, and H1/C1/P1/fresh-C2/fresh-H2 for nested manual navigation; `EXP-129` produced no backend event | Occurrence mechanics, once-per-container integration shape, required presentation styles, nested manual semantics, and internal sibling isolation pass experimentally. Reviewed API, human gestures, same-key hardware acceptance, adaptive navigation, simultaneous-visible hardware, and restoration remain |
...
| Resources and traces | A generic request/span has no intrinsic scene; representative fallback can be wrong | Branch has bounded UI-event/TaskLocal provenance, frozen owners, exact manual Resource start, owner-routed completion, automatic URLSession completion handoff, native/OpenTelemetry span-start parity, and request-time context for completion-created Trace-only URLSession spans; detached/shared work remains ambiguous | Structured/UI-local work retains ownership; delayed completion survives source-scene closure; source-less schedulers use representative fallback. `EXP-133` completes one held Trace-only request after A→B representative churn; `EXP-134` completes independent A/B requests B-before-A while the opposite scene is representative | Existing backend runs show RUM/APM agreement and teardown ownership. `EXP-133` finds one span on A/H1 and zero on B/H1. `EXP-134` finds one A span on A/H1 and one B span on B/H1, zero opposite matches, one shared session, and no matching RUM Resources | Partial and bounded; Trace-only owner freezing and independent reverse completion are backend-pass, while simultaneous exact-source discovery, SwiftUI Button/Task, shared/coalesced completion, compatibility, and overhead gates stay open |
