...
tests (`EXP-108` through `EXP-115`). Three clean iPadOS 27 Home → Detail → Home runs each
...
| --- | --- | --- |
| Concurrent view creation and lifetime | UIKit and explicit SwiftUI windows coexist in one session; the iOS 27 early-mount candidate preserves exact A/B lifecycle work; route-owned controls publish semantic roots and destinations; `EXP-090` proves a RUM-only occurrence change preserves customer SwiftUI state, `EXP-098` reveals retained Home before immediate work, and `EXP-105` preserves that occurrence across a subtree remount. `EXP-115` keeps automatic tracking enabled while an active explicit subtree owns H1/D1/H2 without duplicates; `EXP-116` moves the path and resolver to one probe-container call site | Take the proven container/builder boundary through API review, prove an independent automatic-only container/scene remains tracked, then complete simultaneous visibility, construction, restoration, and iPhone Duo validation |
...
once-per-container configuration can preserve occurrence semantics. A live
separate automatic-only container/scene is still required. `EXP-117` keeps the
