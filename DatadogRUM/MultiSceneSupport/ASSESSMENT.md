...
| Action attribution | UIKit/SwiftUI taps emit once on the source view; SwiftUI scroll, action rejection, action expiry, and overlapping-action fixes have backend or focused-test proof; `EXP-088` makes execution-local manual action calls prefer the exact handoff view/scene; `EXP-089` backend-proves filtered physical capture and later last-interacted fallback | Repeat the manual-handoff run with both windows visible and a deliberately different representative; finish UIKit scroll through deceleration |
...
| Resources and traces | A generic request/span has no intrinsic scene; representative fallback can be wrong | Branch has an experimental `sendEvent`/TaskLocal causal bridge, synchronous owner capture, and frozen completion routing. `EXP-088` consumes exact handoff view/scene for every manual Resource-start overload and retains key-owned completion; GCD, detached, timer, repository, lifecycle, and shared work remain ambiguous | Structured tasks and UI-local work retain source ownership; detached/GCD/timer and early scene lifecycle use the representative. Delayed UIKit/SwiftUI resources and traces survive source-scene closure; `EXP-089` exercises a manual start and post-scope GCD fallback | `EXP-006`, `EXP-009`, and `EXP-019` show RUM/APM agreement and teardown ownership; `EXP-089` retains both Resource completions on A, which was representative before the tap | Partial and bounded; live manual capture/owner completion pass, exact differing-representative precedence remains focused-test-only, and generic source discovery remains impossible |
