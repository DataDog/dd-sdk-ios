...
scene registry, and observable stack/split/UIKit/lifecycle driver now pass 49/49
tests (`EXP-108` through `EXP-118`). Three clean iPadOS 27 Home → Detail → Home runs each
...
uninstall and artifact validation.
Real-device and
...
| Automatic SwiftUI view lifetime | Transparent hosting-controller discovery can select process-global history, run after lifecycle work, or miss semantic route changes | Core scene routing accepts discovered controllers; the route-owned prototype adds keyed state, atomic replacement, disconnect fencing, retained-reader handling, and a weak per-window committed-occurrence source without changing customer content identity. `EXP-115` adds a weak active-subtree authority registry that leaves automatic tracking enabled outside the containing controller and preserves UIKit precedence. `EXP-116` moves path observation and metadata resolution to one probe container while retaining destination-builder placement. `EXP-118` adds scene-selective semantic enablement and an oracle that separates call-site source from mapper ownership and rejects stale automatic views observed before B opens | Automatic baselines remain wrong. Route-owned placement passes initial roots/destinations; `EXP-090` passes same-type replacement with one customer state token; `EXP-091` rejects an unmaterialized abort; `EXP-098`/`EXP-099` reveal retained Home before immediate work; `EXP-102`/`EXP-103` pass same-type split replacement in one and two windows; `EXP-105` preserves a source-started returned split occurrence across remount; `EXP-111` deterministically reruns split replacement under both modes and the automatic control fails 0/9. `EXP-115` runs automatic and explicit tracking together and passes 7/7 with only H1/D1/H2. `EXP-116` repeats return, abort, and same-type replacement through the one-container wrapper. Both clean `EXP-118` prefixes keep A exact and create B-specific fallback/navigation-host views after B opens, but simulator `backboardd` aborts prevent the terminal B marker. The probe passes 49/49; authority/handler regressions add four RUM tests | Backend distinguishes wrong transparent baselines from correct debug-source stack and split chains, including exact immediate markers. In `EXP-111`, automatic Detail₁ uses ApplicationLaunch and Detail₂/Placeholder share one internal hosting UUID. `EXP-115` intake has exactly launch plus H1/D1/H2 and no automatic duplicate; all three isolated `EXP-116` runs match their local oracle. `EXP-118` has no acceptance query because neither run reached a terminal oracle result | Automatic discovery remains the zero-code default and internal target-scoped duplicate suppression passes. The container-level semantics now have a viable probe shape and the separate-scene prefix proves target scoping, but its public API, terminal physical-device coexistence proof, and broader normal-app validation remain |
| SwiftUI navigation | Hosting controllers from all windows share history, transparent discovery may report containers instead of customer screens, and cancellation can publish an uncommitted path | Scene-keyed scope plus a bound typed path drives RUM-only occurrence generations; retained routes use last concrete scene proof only as guarded fallback; arbiter completion owns commit/cancel; a replacement tracking state can adopt a source-published occurrence. The `EXP-116` wrapper owns root/destination materialization and centralizes route metadata once per stack | Explicit repeated push/pop and native/backend cancellation controls pass; debug source passes different-type, same-type, abort, retained Home, same-type split replacement, and retained split return without `.id`. `EXP-111` drives the split cases through observed selection/destination signals and passes 10/10 and 13/13. `EXP-116` passes return 7/7, abort 5/5, and same-type replacement 6/6 through the container wrapper. `EXP-100` gestures were ignored, so native interactive source delivery is still unproven; `EXP-103` ended with a simulator `backboardd` crash after telemetry acceptance | `EXP-090` emits distinct Detail UUIDs without customer-state reset; `EXP-098` emits Home₁ → Detail → Home₂ with exact immediate work; `EXP-102`/`EXP-103` emit exact split Detail₁ → Detail₂ → Placeholder chains; `EXP-105` adds one exact returned-Detail occurrence; `EXP-111` independently repeats those split chains with local oracle and backend agreement; isolated `EXP-116` runs preserve exact return, abort, and replacement view sets and downstream owners | Occurrence mechanics and a once-per-container integration shape pass experimentally; review the public API, then cover human-driven gestures, adaptive navigation, separate automatic containers, simultaneous-visible hardware, and restoration |
