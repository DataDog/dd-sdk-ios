...
| --- | --- | --- |
| Concurrent view creation and lifetime | UIKit and explicit SwiftUI windows coexist in one session; the iOS 27 early-mount candidate preserves exact A/B lifecycle work; route-owned controls publish semantic roots and destinations; `EXP-090` proves a RUM-only occurrence change preserves customer SwiftUI state, `EXP-098` reveals retained Home before immediate work, and `EXP-105` preserves that occurrence across a subtree remount. `EXP-115` keeps automatic tracking enabled while an active explicit subtree owns H1/D1/H2 without duplicates | Finish the approved once-per-container path/router integration and centralized resolver, prove an independent automatic-only container/scene remains tracked, then complete simultaneous visibility, construction, restoration, and iPhone Duo validation |
...
The debug source still needs the approved once-per-container path/router and
centralized resolver, plus a live separate automatic-only container/scene.
Real-device and
...
| Automatic SwiftUI view lifetime | Transparent hosting-controller discovery can select process-global history, run after lifecycle work, or miss semantic route changes | Core scene routing accepts discovered controllers; the route-owned prototype adds keyed state, atomic replacement, disconnect fencing, retained-reader handling, and a weak per-window committed-occurrence source without changing customer content identity. `EXP-115` adds a weak active-subtree authority registry that leaves automatic tracking enabled outside the containing controller and preserves UIKit precedence | Automatic baselines remain wrong. Route-owned placement passes initial roots/destinations; `EXP-090` passes same-type replacement with one customer state token; `EXP-091` rejects an unmaterialized abort; `EXP-098`/`EXP-099` reveal retained Home before immediate work; `EXP-102`/`EXP-103` pass same-type split replacement in one and two windows; `EXP-105` preserves a source-started returned split occurrence across remount; `EXP-111` deterministically reruns split replacement under both modes and the automatic control fails 0/9. `EXP-115` runs automatic and explicit tracking together and passes 7/7 with only H1/D1/H2. Authority/handler regressions add four tests | Backend distinguishes wrong transparent baselines from correct debug-source stack and split chains, including exact immediate markers. In `EXP-111`, automatic Detail₁ uses ApplicationLaunch and Detail₂/Placeholder share one internal hosting UUID. `EXP-115` intake has exactly launch plus H1/D1/H2 and no automatic duplicate | Automatic discovery remains the zero-code default and internal target-scoped duplicate suppression passes. Exact semantics still require the reviewed once-per-container path/router integration, centralized resolver, separate automatic-container runtime proof, and broader normal-app validation |
