...
class RUMViewScope_Tests: XCTestCase {
    ...
    func DISABLED_testWhenViewIsStopped_itSendsViewUpdateEvent_andEndsTheScope() throws {
        ...
        XCTAssertEqual(writer.events(ofType: RUMViewUpdateEvent.self).count, 1)
        writer.events(ofType: RUMViewEvent.self).forEach { viewEvent in
            XCTAssertEqual(
                viewEvent.date,
        ...
        writer.events(ofType: RUMViewUpdateEvent.self).forEach { viewEvent in
            XCTAssertEqual(
                viewEvent.date,
                Date.mockDecember15th2019At10AMUTC().timeIntervalSince1970.dd.toInt64Milliseconds,
                "All View events must share the same creation date"
        ...
        XCTAssertEqual(event.view.action?.count ?? 0, 0)
        XCTAssertEqual(event.view.error?.count ?? 0, 0)
    ...
    func DISABLED_testWhenAnotherViewIsStarted_itEndsTheScope() throws {
        ...
        XCTAssertEqual(writer.events(ofType: RUMViewEvent.self).count, 1)
        XCTAssertEqual(writer.events(ofType: RUMViewUpdateEvent.self).count, 1)
        let firstViewEvent = try XCTUnwrap(writer.events(ofType: RUMViewEvent.self).first)
    ...
    func DISABLED_testWhenTheViewIsStartedAnotherTime_itEndsTheScope() throws {
        ...
        let firstViewEvent = try XCTUnwrap(writer.events(ofType: RUMViewEvent.self).first)
        let stopViewEvent = try XCTUnwrap(writer.events(ofType: RUMViewUpdateEvent.self).first)
        let viewWasActive = try XCTUnwrap(firstViewEvent.view.isActive)
        XCTAssertTrue(viewWasActive)
        XCTAssertEqual(firstViewEvent.view.url, "FirstViewController")
        XCTAssertEqual(firstViewEvent.view.name, "FirstViewName")
        let viewIsActive = try XCTUnwrap(stopViewEvent.view.isActive)
        XCTAssertFalse(viewIsActive)
    ...
    func DISABLED_testGivenMultipleViewScopes_whenSendingViewEvent_eachScopeUsesUniqueViewID() throws {
        ...
        // Then
        let view1StartEvent = try XCTUnwrap(writer.events(ofType: RUMViewEvent.self).first { $0.view.url == "View1URL" })
        let view1UpdateEvent = try XCTUnwrap(writer.events(ofType: RUMViewUpdateEvent.self).first { $0.view.url == "View1URL" })
        let view2StartEvent = try XCTUnwrap(writer.events(ofType: RUMViewEvent.self).first { $0.view.url == "View2URL" })
        let view2UpdateEvent = try XCTUnwrap(writer.events(ofType: RUMViewUpdateEvent.self).first { $0.view.url == "View2URL" })
        XCTAssertEqual(view1StartEvent.view.id, view1UpdateEvent.view.id)
        XCTAssertEqual(view2StartEvent.view.id, view2UpdateEvent.view.id)
        XCTAssertNotEqual(view1StartEvent.view.id, view2StartEvent.view.id)
    }

    func DISABLED_testWhenEventsAreSent_theyIncludeSessionPrecondition() throws {
        ...
        XCTAssertGreaterThan(
            writer.events(ofType: RUMViewEvent.self).count + writer.events(ofType: RUMViewUpdateEvent.self).count,
            1
        )
        writer.events(ofType: RUMViewEvent.self).forEach { XCTAssertEqual($0.dd.session?.sessionPrecondition, randomPrecondition) }
        writer.events(ofType: RUMViewUpdateEvent.self).forEach { XCTAssertEqual($0.dd.session?.sessionPrecondition, randomPrecondition) }
    ...
    func DISABLED_testWhenViewAttributesAreSet_childEventsHaveThem() throws {
        let scope: RUMViewScope_ = .mockWith(parent: parent)
        ...
        // Then
        let updateViewEvent = try XCTUnwrap(writer.events(ofType: RUMViewUpdateEvent.self).last)
        let actionEvent = try XCTUnwrap(writer.events(ofType: RUMActionEvent.self).last)
        XCTAssertNil(scope.userActionScope)
        DDAssertDictionariesEqual(scope.attributes, ["viewKey": "viewValue"])
        DDAssertDictionariesEqual(updateViewEvent.context!.contextInfo, ["viewKey": "viewValue", "globalKey": "globalValue"])
        ...
        // Then
        let updateViewEvent2 = try XCTUnwrap(writer.events(ofType: RUMViewUpdateEvent.self).last)
        ...
        DDAssertDictionariesEqual(updateViewEvent2.context!.contextInfo, ["viewKey": "viewValue", "newViewKey": "newViewValue", "globalKey": "globalValue"])
    ...
    func DISABLED_testWhenViewAttributesCollide_thePrecedenceIsRespected() throws {
        let scope: RUMViewScope_ = .mockWith(parent: parent)
        ...
        // Then
        let updateViewEvent1 = try XCTUnwrap(writer.events(ofType: RUMViewUpdateEvent.self).last)
        let actionEvent = try XCTUnwrap(writer.events(ofType: RUMActionEvent.self).last)
        DDAssertDictionariesEqual(scope.attributes, ["key": "viewValue"])
        DDAssertDictionariesEqual(updateViewEvent1.context!.contextInfo, ["key": "viewValue"])
        ...
        let resourceEvent = try XCTUnwrap(writer.events(ofType: RUMResourceEvent.self).last)
        DDAssertDictionariesEqual(scope.attributes, ["key": "viewValue"])
        DDAssertDictionariesEqual(updateViewEvent2.context!.contextInfo, ["key": "viewValue"])
    ...
    func DISABLED_testWhenViewAttributesChangeOnStoppedViewWithActiveResources() throws {
        let view1 = "view1"
        let view2 = "view2"
        let firstViewScope: RUMViewScope_ = .mockWith(parent: parent, identity: ViewIdentifier(view1), name: view1)
        ...
        let secondViewScope: RUMViewScope_ = .mockWith(parent: parent, identity: ViewIdentifier(view2), name: view2)
        XCTAssertTrue(secondViewScope.process(command: startView2Command, context: context, writer: writer))

        // Then
        let updateView1Event = try XCTUnwrap(writer.events(ofType: RUMViewUpdateEvent.self).last(where: { $0.view.name == "view1" }))
        ...
        DDAssertDictionariesEqual(updateView1Event.context!.contextInfo, ["viewKey": "viewValue", "newViewKey": "newViewValue", "globalKey": "globalValue"])
        ...
        // Then
        let updateView1Event2 = try XCTUnwrap(writer.events(ofType: RUMViewUpdateEvent.self).last)
        let resourceEvent = try XCTUnwrap(writer.events(ofType: RUMResourceEvent.self).last)
        // First view scope is inactive with the final snapshot of attributes
        DDAssertDictionariesEqual(firstViewScope.attributes, ["viewKey": "viewValue", "newViewKey": "newViewValue", "globalKey": "globalValue"])
        DDAssertDictionariesEqual(secondViewScope.attributes, ["view2Key": "view2Value", "newView2Key": "newView2Value"])
        DDAssertDictionariesEqual(
            updateView1Event2.context!.contextInfo,
            [
                "viewKey": "viewValue",
                "newViewKey": "newViewValue",
                "globalKey": "globalValue"
    ...
    func DISABLED_testItManagesResourceScopesLifecycle() throws {
        let scope = RUMViewScope_(
            isInitialView: .mockRandom(),
            parent: parent,
            dependencies: .mockAny(),
    ...
    func DISABLED_testGivenViewWithPendingResources_whenItGetsStopped_itDoesNotFinishUntilResourcesComplete() throws {
        ...
        let updateEvent = try XCTUnwrap(writer.events(ofType: RUMViewUpdateEvent.self).last)
        XCTAssertEqual(updateEvent.view.resource?.count, 1, "View should record 1 successful Resource")
        XCTAssertEqual(updateEvent.view.error?.count, 1, "View should record 1 error due to second Resource failure")
        XCTAssertFalse(updateEvent.view.isActive ?? true, "View should be inactive")
        XCTAssertEqual(updateEvent.view.timeSpent, lastResourceCompletionTime.timeIntervalSince(viewStartTime).dd.toInt64Nanoseconds, "View should last until the last resource completes")
    }

    func DISABLED_testGivenViewWithUnfinishedResources_whenNextViewsAreStarted_itNoLongerUpdatesTimeSpent() throws {
    ...
    func DISABLED_testItManagesContinuousUserActionScopeLifecycle() throws {
        let scope = RUMViewScope_(
            isInitialView: false,
            parent: parent,
            dependencies: .mockAny(),
    ...
    func DISABLED_testItManagesDiscreteUserActionScopeLifecycle() throws {
        var currentTime = Date()
        let scope = RUMViewScope_(
            isInitialView: false,
            parent: parent,
    ...
    func DISABLED_testGivenViewWithPendingAction_whenCustomActionIsAdded_itSendsItInstantly() throws {
        var currentTime = Date()
        let scope = RUMViewScope_(
            isInitialView: false,
            parent: parent,
    ...
    func DISABLED_testGivenViewWithNoPendingAction_whenCustomActionIsAdded_itSendsItInstantly() throws {
        var currentTime = Date()
        let scope = RUMViewScope_(
            isInitialView: false,
            parent: parent,
    ...
    func DISABLED_testWhenDiscreteUserActionHasFrustration_itSendsFrustrationCount() throws {
        // Given
        var currentTime = Date()
        let scope = RUMViewScope_(
            isInitialView: false,
    ...
    func DISABLED_testWhenTwoTapActionsTrackedSequentially_thenHigherPriorityInstrumentationWins() throws {
    ...
    func DISABLED_testWhenViewErrorIsAdded_itSendsErrorEventAndViewUpdateEvent() throws {
    ...
    func DISABLED_testWhenViewErrorIsAddedWithConfiguredSource_itSendsErrorEventWithCorrectSource() throws {
        var currentTime: Date = .mockDecember15th2019At10AMUTC()
        let source = String.mockAnySource()
    ...
    func DISABLED_testGivenStartedView_whenCrossPlatformErrorIsAdded_itSendsCorrectErrorEvent() throws {
        let completionExpectation = expectation(description: "Error processing completion")
        ...
        let scope: RUMViewScope_ = .mockWith(
            parent: parent,
            dependencies: .mockAny()
        )
    ...
    func DISABLED_testGivenStartedView_whenErrorWithAttributesIsAdded_itDoesNotUpdateViewAttributes() throws {
        let hasReplay: Bool = .mockRandom()
        var context = self.context
        context.set(additionalContext: SessionReplayCoreContext.HasReplay(value: hasReplay))
    ...
    func DISABLED_testGivenStartedView_whenErrorWithFingerprintAttributesIsAdded_itAddsFingerprintToError() throws {
        // Given
        let hasReplay: Bool = .mockRandom()
    ...
    func DISABLED_testGivenStartedView_whenErrorWithIncludeBinaryImagesAttributesIsAdded_itAddsBinaryImagesToError() throws {
        // Given
    ...
    func DISABLED_testWhenResourceIsFinishedWithError_itSendsViewUpdateEvent() throws {
        let scope = RUMViewScope_(
            isInitialView: .mockRandom(),
            parent: parent,
            dependencies: .mockAny(),
    ...
    func DISABLED_testWhenViewErrorIsAdded_itSendsErrorWithCorrectTimeSinceAppStart() throws {
        var context = self.context
        let currentTime: Date = .mockDecember15th2019At10AMUTC()
    ...
    func DISABLED_testWhenThereAreHitches_theViewUpdatesContainsSlowFrames() {
        // Given
        var hitches: [Hitch] = []
        ...
        // Then
        XCTAssertEqual(writer.events(ofType: RUMViewEvent.self).count, 1)
        XCTAssertEqual(writer.events(ofType: RUMViewUpdateEvent.self).count, 5)
    ...
    func DISABLED_testWhenAppHangsAndViewHitchesAreDisabled_theRatesAreNotCalculated() {
        // Given
        var currentTime: Date = .mockDecember15th2019At10AMUTC()
        let scope = RUMViewScope_(
            isInitialView: .mockRandom(),
        ...
        XCTAssertEqual(writer.events(ofType: RUMViewEvent.self).count, 1)
        XCTAssertEqual(writer.events(ofType: RUMViewUpdateEvent.self).count, 1)
        let stopViewEvent = writer.events(ofType: RUMViewUpdateEvent.self).last
    ...
    func DISABLED_testWhenViewDurationIsTooSmall_theRatesAreNotCalculated() {
        // Given
        var currentTime: Date = .mockDecember15th2019At10AMUTC()
        var hitches: [Hitch] = []
        (0...Int.mockRandom(min: 0, max: 1_000)).forEach {
    ...
    func DISABLED_testWhenThereAreViewHitches_theStopViewEventHasSlowFramesRate() {
        // Given
        var currentTime: Date = .mockDecember15th2019At10AMUTC()
        var hitches: [Hitch] = []
        (0..<10).forEach {
        ...
        // Then
        XCTAssertEqual(writer.events(ofType: RUMViewEvent.self).count, 1)
        XCTAssertEqual(writer.events(ofType: RUMViewUpdateEvent.self).count, 2)
        XCTAssertNil(writer.events(ofType: RUMViewEvent.self).first?.view.slowFramesRate)
        XCTAssertNil(writer.events(ofType: RUMViewUpdateEvent.self).first?.view.slowFramesRate)
        let stopViewEvent = writer.events(ofType: RUMViewUpdateEvent.self).last
        XCTAssertEqual(stopViewEvent?.view.slowFrames?.count, hitches.count)
    ...
    func DISABLED_testWhenThereAreAppHangs_theStopViewEventHasFreezeRate() {
        // Given
        var currentTime: Date = .mockDecember15th2019At10AMUTC()
        let scope = RUMViewScope_(
            isInitialView: .mockRandom(),
        ...
        XCTAssertEqual(writer.events(ofType: RUMViewEvent.self).count, 1)
        XCTAssertEqual(writer.events(ofType: RUMViewUpdateEvent.self).count, 2)
        XCTAssertNil(writer.events(ofType: RUMViewEvent.self).first?.view.freezeRate)
        XCTAssertNil(writer.events(ofType: RUMViewUpdateEvent.self).first?.view.freezeRate)
        // The rate is only calculated in the Stop View event
        let stopViewEvent = writer.events(ofType: RUMViewUpdateEvent.self).last
        XCTAssertEqual(stopViewEvent?.view.freezeRate, 0.5.hours)
    }

    func DISABLED_testWhenViewErrorIsAdded_ButErrorEventDiscarded_itCallsCompletionHandler() throws {
        let completionExpectation = expectation(description: "Error processing completion")
    ...
    func DISABLED_testWhenViewAppHangIsTracked_itSendsErrorEventAndViewUpdateEvent() throws {
        var currentTime: Date = .mockDecember15th2019At10AMUTC()
    ...
    func DISABLED_testWhenLongTaskIsAdded_itSendsLongTaskEventAndViewUpdateEvent() throws {
        let hasReplay: Bool = .mockRandom()
        var context = self.context
        context.set(additionalContext: SessionReplayCoreContext.HasReplay(value: hasReplay))
    ...
    func DISABLED_testGivenStartedView_whenLongTaskWithAttributesIsAdded_itDoesNotUpdateViewAttributes() throws {
        let hasReplay: Bool = .mockRandom()
        var context = self.context
        context.set(additionalContext: SessionReplayCoreContext.HasReplay(value: hasReplay))
    ...
    func DISABLED_testWhenLongTaskIsAddedWithConfiguredSource_itSendsLongTaskEventWithConfiguredSource() throws {
        let startViewDate: Date = .mockDecember15th2019At10AMUTC()
    ...
    func DISABLED_testGivenActiveView_whenCustomTimingIsRegistered_itSendsViewUpdateEvent() throws {
        var currentTime: Date = .mockDecember15th2019At10AMUTC()
        let scope = RUMViewScope_(
            isInitialView: .mockRandom(),
            parent: parent,
    ...
    func DISABLED_testGivenActiveView_whenCustomTimingIsRegistered_itSanitizesCustomTiming() throws {
        var currentTime: Date = .mockDecember15th2019At10AMUTC()
        let scope = RUMViewScope_(
            isInitialView: .mockRandom(),
            parent: parent,
    ...
    func DISABLED_testGivenActiveView_whenFeatureFlagEvaluated_itAddsTheFeatureFlag() throws {
        // Given
        let currentTime: Date = .mockDecember15th2019At10AMUTC()
        let scope = RUMViewScope_(
            isInitialView: .mockRandom(),
    ...
    func DISABLED_testGivenActiveView_whenFeatureFlagReEvaluated_itModifiesTheFeatureFlag() throws {
        // Given
        let currentTime: Date = .mockDecember15th2019At10AMUTC()
        let scope = RUMViewScope_(
            isInitialView: .mockRandom(),
    ...
    func DISABLED_testGivenActiveViewWithFeatureFlag_itSendsThoseFlagsWithErrors() throws {
        // Given
        let currentTime: Date = .mockDecember15th2019At10AMUTC()
        let scope = RUMViewScope_(
            isInitialView: .mockRandom(),
    ...
    func DISABLED_testGivenViewStartedWithServerTimeDifference_whenDifferentEventsAreSend_itAppliesTheSameCorrectionToAll() throws {
        let initialDeviceTime: Date = .mockDecember15th2019At10AMUTC()
        let initialServerTimeOffset: TimeInterval = 120 // 2 minutes
        ...
        writer.events(ofType: RUMViewEvent.self).forEach { view in
            XCTAssertEqual(view.date, expectedViewEventsDate)
        }
        writer.events(ofType: RUMViewUpdateEvent.self).forEach { view in
            XCTAssertEqual(view.date, expectedViewEventsDate)
        }
    ...
    func DISABLED_testGivenViewScopeWithDependentActionsResourcesErrors_whenDroppingEvents_thenCountsAreAdjusted() throws {
    ...
    @MainActor
    func DISABLED_testAccessibilityAttributesInViewEvents() throws {
        // Given
