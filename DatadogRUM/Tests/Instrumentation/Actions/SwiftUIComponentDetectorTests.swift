...
class SwiftUIComponentDetectorTests: XCTestCase {
    ...
    private let defaultPredicate = DefaultSwiftUIRUMActionsPredicate()
    ...
    func testModernDetector_DetectsNavigationLinkFromGestureName() {
        // Given
        let detector = ModernSwiftUIComponentDetector()
        let defaultPredicate = DefaultSwiftUIRUMActionsPredicate()
        let mockTouch = MockUITouch(
            phase: .began,
            view: SwiftUIViewMock(),
        ...
        mockTouch.mockGestures = [
            MockGestureRecognizer(name: "com.apple.UIKit.HomeAffordanceGate")
        ]
    ...
    func testModernDetector_DetectsToggle() {
        ...
        let grandparentView = MockToggleView()
        grandparentView.mockTypeDescription = "UISwitch"
    ...
    @available(iOS 18.0, *)
    func testModernDetector_IgnoresUnsafePrivacyViews() {
        // Given
        let detector = ModernSwiftUIComponentDetector()
        let unsafeView = SwiftUIViewMock()
        unsafeView.overrideIsSafeForPrivacy = false
        let mockTouch = MockUITouch(
            phase: .began,
            view: unsafeView,
            description: "ButtonGesture"
    ...
    func testLegacyDetector_IgnoresHostingViews() {
        // Given
        let detector = LegacySwiftUIComponentDetector()
        let hostingView = HostingViewMock()
        let mockTouch = MockUITouch(phase: .ended, view: hostingView)
    ...
    func testLegacyDetector_IgnoresHostingScrollViews() {
        // Given
        let detector = LegacySwiftUIComponentDetector()
        let hostingScrollView = HostingScrollViewMock()
        let mockTouch = MockUITouch(phase: .ended, view: hostingScrollView)
    ...
    func testLegacyDetector_IgnoresUnsafePrivacyViews() {
        // Given
        let detector = LegacySwiftUIComponentDetector()
        let unsafeView = SwiftUIViewMock()
        unsafeView.overrideIsSafeForPrivacy = false
    ...
    func testSwiftUIComponentHelpers_ExtractComponentName() {
        ...
        mockTouch.mockGestures = [MockGestureRecognizer(name: "com.apple.UIKit.HomeAffordanceGate")]
        XCTAssertEqual(
            SwiftUIComponentHelpers.extractComponentName(touch: mockTouch, defaultName: "default"),
...
private class HostingViewMock: SwiftUIViewMock {
    override var typeDescription: String {
        return "SwiftUI.HostingView"
    }
...
private class HostingScrollViewMock: SwiftUIViewMock {
    override var typeDescription: String {
        return "SwiftUI.HostingScrollView"
    }
