...
extension DatadogExtension where ExtendedType: UIImage {
    var srIdentifier: String {
        if let hash = objc_getAssociatedObject(self.type, &srIdentifierKey) as? String {
            return hash
        } else {
            let hash = computeHash()
            objc_setAssociatedObject(self.type, &srIdentifierKey, hash, .OBJC_ASSOCIATION_RETAIN)
            return hash
        }
    }
