...
extension SRWireframe: Diffable {
    var id: DiffableID {
        ...
        case .webviewWireframe(let wireframe):
            return wireframe.id
        case .embeddedViewWireframe(let wireframe):
            return wireframe.id
        }
    }
    ...
    func isDifferent(than otherElement: SRWireframe) -> Bool {
        ...
        case let (.webviewWireframe(this), .webviewWireframe(other)):
            return this.hashValue != other.hashValue
        case let (.embeddedViewWireframe(this), .embeddedViewWireframe(other)):
            return this.hashValue != other.hashValue
        default:
            return true
        }
    ...
    var type: String {
        ...
        case let (.webviewWireframe(value)):
            return value.type
        case let (.embeddedViewWireframe(value)):
            return value.type
        }
    }
}
...
extension SRWireframe: MutableWireframe {
    func mutations(from otherWireframe: SRWireframe) throws -> WireframeMutation {
        ...
        case .webviewWireframe(let this):
            return try this.mutations(from: otherWireframe)
        case .embeddedViewWireframe(let this):
            return try this.mutations(from: otherWireframe)
        }
    }
}

extension SREmbeddedViewWireframe: MutableWireframe {
    func mutations(from otherWireframe: SRWireframe) throws -> WireframeMutation {
        guard case .embeddedViewWireframe(let other) = otherWireframe else {
        ...
        return .embeddedViewWireframeUpdate(
            value: .init(
