...
extension SessionReplayWireframesBuilder {
    ...
    public func visibleEmbeddedViewWireframe(
        id: Int,
        frame: CGRect,
        clip: CGRect,
        borderColor: CGColor? = nil,
        ...
        let wireframe = SREmbeddedViewWireframe(
            border: createShapeBorder(borderColor: borderColor, borderWidth: borderWidth),
            clip: SRContentClip(frame, intersecting: clip),
            height: Int64.ddWithNoOverflow(frame.height),
            id: Int64(id),
        ...
        return .embeddedViewWireframe(value: wireframe)
    }

    public func hiddenEmbeddedViewWireframes() -> [SRWireframe] {
        if !flutterViewSlotIDs.isEmpty {
            NSLog("[DD-SR] hiddenEmbeddedViewWireframes emitting \(flutterViewSlotIDs.count) hidden slot(s): \(flutterViewSlotIDs)")
        }
