#!/usr/bin/env python3
"""Run frozen retained reader remount control/candidate apps."""
import argparse
...
for mode in ['explicit', 'capability']:
    REQUIRED_CHECKS.update(mode + '_' + suffix for suffix in [
        ...
        'removed_native_window', 'detach_releases', 'source_unsubscribed',
        'input_while_detached_no_owner', 'peer_unchanged', 'retained_reader_still_exists',
        'existing_callback_ready', 'first_mount_intercepted', 'first_mount_before_body',
        'first_mount_source_scene', 'source_reobserved', 'authority_restored',
        'latest_owner', 'fresh_id', 'mount_resource_owner', 'mount_log_owner',
        'duplicate_reader_no_change', 'peer_survives_mount', 'final_detach_releases',
        'source_unsubscribed_final', 'two_owned_occurrences', 'one_peer_occurrence',
        'no_intermediate_home'])
...
def prepare(root, arm, revision):
    ...
    bundle = 'com.datadoghq.retained-reader.' + arm
    info = {
        'CFBundleName': 'RetainedReader', 'CFBundleIdentifier': '$(PRODUCT_BUNDLE_IDENTIFIER)',
    ...
    project = {
        'name': 'RetainedReader', 'packages': {'SDK': {'path': 'sdk'}},
        'targets': {'RetainedReader': {
        ...
        'schemes': {'RetainedReader': {'build': {'targets': {'RetainedReader': 'all'}}}}}
...
def build(root, arm, item):
    directory = root / arm
    command = ['xcodebuild', 'build', '-quiet', '-project', str(directory / 'RetainedReader.xcodeproj'),
               '-scheme', 'RetainedReader', '-configuration', 'Debug',
    ...
    app = directory / 'derived/Build/Products/Debug-iphonesimulator/RetainedReader.app'
    item.update(app=str(app), binary_sha256=helpers.digest(app / 'RetainedReader'),
...
def execute(root, arm, item, device):
    ...
    bundle = item['bundle']
    app = Path(item['app'])
    if helpers.digest(app / 'RetainedReader') != item['binary_sha256']:
    ...
    if helpers.digest(installed / 'RetainedReader') != item['binary_sha256']:
    ...
    run_id = 'retained-reader-' + uuid.uuid4().hex
    env = ENV
    ...
    setup_valid = complete and not result.get('setup_failure') and all(
        checks.get(key) for key in ['native_scene_active', 'explicit_mounted', 'explicit_registry_present', 'explicit_reader_present', 'explicit_first_mount_intercepted', 'explicit_first_mount_before_body', 'capability_mounted', 'capability_registry_present', 'capability_reader_present', 'capability_first_mount_intercepted', 'capability_first_mount_before_body'])
...
def main():
    ...
    root = Path(tempfile.mkdtemp(prefix='retained-reader-'))
    print(str(root), flush=True)
    manifest = {'artifact_root': str(root), 'experiment': 'EXP-171', 'status': 'INVALID', 'arms': {}}
    try:
        ...
        for arm, item in manifest['arms'].items():
            ...
            item['binary_unchanged'] = helpers.digest(Path(item['app']) / 'RetainedReader') == item['binary_sha256']
        ...
        control_detected = control['status'] == 'FAIL' and all(
            control['result']['checks'][mode + '_latest_owner'] is False
            for mode in ['explicit', 'capability'])
