...
- Current SDK checkpoint is signed `368c62a72` (EXP-174 observer delivery);
...
- **Current executable task:** execute defined EXP-175 for the existing T03 late
  Resource completion regression, keeping a new session continuous action alive.
  Then finish T03 explicit-start/URLSession/backend acceptance before another
  telemetry family. The full gate stays open until that evidence passes. Do not expand T03–T14 before relevant dependencies pass. Deferred
...
5. EXP-160 through EXP-175 in [EXP-143-199.md](/Users/valentin.pertuisot/work/dd-sdk-ios/DatadogRUM/MultiSceneSupport/Experiments/EXP-143-199.md); inspect EXP-159 only for a relevant regression.
...
execute defined EXP-175, then finish the T03 target/backend slice; baseline and acceptance tooling
