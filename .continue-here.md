...
- Current SDK checkpoint is signed `5e41d0b11` (EXP-176 prepared Resource starts);
...
- Current finite release register: **30/66 gates closed**. Read PLAN and
...
- EXP-176 SDK is prepared at signed `5e41d0b11`: ten new regressions,397
  affected tests across396+1 passes,8 Objective-C API checks, Release and strict
  lint. Explicit starts work; ordinary session restoration keeps its representative;
  response-plus-error URLSession completion emits one error. Failed controls, test
  constructor/import and legacy-host limitations remain in its durable result.
- **Current executable task:** finish EXP-176 native Resource/URLSession mapper and
  backend acceptance with the existing A01 harness before another telemetry family. The full gate
  stays open until that evidence passes. Do not expand T03–T14 before relevant dependencies pass. Deferred
