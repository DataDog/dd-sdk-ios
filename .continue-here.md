...
- Current SDK checkpoint is signed `1e9c4788d` (EXP-177 errors prepared);
...
- Current finite release register: **31/66 gates closed**. Read PLAN and
...
- EXP-177 SDK preparation passes at signed `1e9c4788d`:3 failing controls,11 new/
  376 affected tests,8 ObjC checks, strict repository lint and iOS/watchOS Release.
  Explicit current-view errors preserve inference and captured Resource owners;
  missing-recipient/unsampled completions now run once after all pending writes.
 168 probe,67 Python and3 connector checks pass. Invalid build/lint/control attempts
  are retained in the result. T04 remains open until native/backend acceptance.
- **Current executable task:** run the frozen EXP-177 current-view error acceptance
  with the named T04 runner contract:9 errors,2 actions,3 views,1 callback,24 local
  expectations and exact backend IDs/counters. SDK/fixture controls are already
  accepted; do not rerun merely to resume. Then close T04 if accepted and define
  T05 view attributes/removal before implementation; T06 follows.
  Deferred extraction remains after freeze; posted notifications never close
  hardware gates. Preserve T03's Resource-error ownership.
