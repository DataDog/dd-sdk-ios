...
- Current SDK checkpoint is signed `5e41d0b11` (EXP-176 accepted Resource starts);
...
- **Current executable task:** define and execute the T04 current-view error
  target contract (next EXP-177), using the accepted A01 runner and retaining
  T03 Resource-error ownership. Audit message/Error/completion-handler forms,
  custom/NOP fallback, independent inference and exact mapper/backend owners
  before implementation. Then T05 attributes/removal and T06 timing/loading.
  Deferred extraction remains after freeze; posted notifications never close
  hardware gates. Do not rerun accepted experiments merely to resume.
